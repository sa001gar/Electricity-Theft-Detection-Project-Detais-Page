...
import { INITIAL_DATA_POINTS, UPDATE_INTERVAL, SYSTEM_VOLTAGE } from '../utils/constants';
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg" onContextMenu={handleContextMenu}>
      ...
      <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-gray-800">LED Power Monitor</h2>
          <p className="text-sm text-gray-600">
            Current Draw: {currentValue.toFixed(3)} A | 
            Power: {(currentValue * SYSTEM_VOLTAGE).toFixed(2)} W
          </p>
        </div>
        <button
      ...
      <div className="mt-4 text-sm text-gray-600 text-center">
        <p>Real-time Power Monitoring System</p>
      </div>
    </div>
  );
};
